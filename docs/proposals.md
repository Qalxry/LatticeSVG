...
### ~~P2-6. hyphenation（单词断字）~~（已完成）

**价值**：在英文文本中启用单词断字（hyphenation）可显著改善 justify 对齐的视觉效果，避免大间隙。对中文无影响。

**代价**：需引入第三方 hyphenation 库（如 `pyphen`），在 shaper 中实现断字逻辑，修改 justify 计算以考虑断字后的单词分割。

**评估**：对英文 justify 有明显提升，但增加了依赖和实现复杂度，按需评估。
